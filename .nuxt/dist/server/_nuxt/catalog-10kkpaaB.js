...
import { f as fetchProductos } from "./useSyscom-Bu-iKqE2.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(search)) {
        _push(`<button style="${ssrRenderStyle({ "position": "absolute", "right": "12px", "top": "50%", "transform": "translateY(-50%)", "background": "rgba(255,255,255,0.08)", "border": "none", "border-radius": "50%", "width": "18px", "height": "18px", "display": "flex", "align-items": "center", "justify-content": "center", "cursor": "pointer", "color": "rgba(148,163,184,0.8)", "font-size": "12px" })}">×</button>`);
      } else {
        _push(`<!---->`);
      }
      ...
      _push(`<span style="${ssrRenderStyle({ "overflow": "hidden", "text-overflow": "ellipsis", "white-space": "nowrap", "max-width": "100px" })}">${ssrInterpolate(unref(activeBrandId) ? unref(brands).find((b) => b.id === unref(activeBrandId))?.nombre ?? unref(activeBrandId) : "Marca")}</span></div>`);
      if (unref(activeBrandId)) {
        _push(`<span style="${ssrRenderStyle({ "font-size": "14px", "line-height": "1", "cursor": "pointer" })}">×</span>`);
      } else {
        _push(ssrRenderComponent(unref(ChevronDown), {
          size: 13,
      ...
      if (unref(loadingCats) || unref(loading) && !unref(products).length && !unref(hasFilter)) {
        ...
        _push(`</div><div style="${ssrRenderStyle({ "font-size": "15px", "font-weight": "600", "color": "#94a3b8", "margin-bottom": "5px" })}">Sin resultados</div><div style="${ssrRenderStyle({ "font-size": "13px", "color": "rgba(71,85,105,0.9)" })}">Prueba con otra búsqueda o categoría</div><button style="${ssrRenderStyle({ "margin-top": "16px", "padding": "8px 18px", "border-radius": "9px", "background": "rgba(14,165,233,0.1)", "border": "1px solid rgba(14,165,233,0.2)", "color": "#38bdf8", "font-size": "12px", "font-weight": "600", "cursor": "pointer", "font-family": "inherit" })}">Limpiar filtros</button></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
//# sourceMappingURL=catalog-10kkpaaB.js.map
