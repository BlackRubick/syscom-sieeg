...
import { p as parseTotal, b as facturaStatusStyle, c as fmtCompact, d as fmtDate } from "./useSyscom-D0TdJReD.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const statusCounts = computed(() => {
      const acc = {};
      facturas.value.forEach((f) => {
        const l = facturaStatusStyle(f.estatus ?? f.estado).label;
        acc[l] = (acc[l] ?? 0) + 1;
      });
      return acc;
    });
    ...
    const kpis = computed(() => [
      { label: `Facturas ${YEAR}`, value: loading.value ? "—" : totalOrders.value.toLocaleString("es-MX"), sub: "vs. año anterior", delta: ordersDelta.value, icon: ShoppingBag, accent: "#0EA5E9", gradient: "linear-gradient(135deg,#0EA5E9,#7DD3FC)", glow: "rgba(14,165,233,0.2)" },
      { label: `Gasto total ${YEAR}`, value: loading.value ? "—" : fmtCompact(totalSpent.value), sub: "vs. año anterior", delta: spentDelta.value, icon: DollarSign, accent: "#8B5CF6", gradient: "linear-gradient(135deg,#0EA5E9,#7DD3FC)", glow: "rgba(139,92,246,0.2)" },
      { label: `Gasto de ${MONTHS[CURRENT_MONTH]}`, value: loading.value ? "—" : monthSpent.value > 0 ? fmtCompact(monthSpent.value) : "$0", sub: `${thisMonth.value.length} factura${thisMonth.value.length !== 1 ? "s" : ""} este mes`, delta: null, icon: CalendarDays, accent: "#22C55E", gradient: "linear-gradient(135deg,#22C55E,#34D399)", glow: "rgba(34,197,94,0.2)" }
    ]);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        _push(`<!--[-->`);
        ssrRenderList(3, (i) => {
          _push(`<div style="${ssrRenderStyle({ "border-radius": "16px", "background": "linear-gradient(145deg,#0C1A2E,#06101E)", "border": "1px solid rgba(255,255,255,0.07)", "padding": "22px 24px" })}"><div style="${ssrRenderStyle({ "display": "flex", "justify-content": "space-between", "align-items": "flex-start", "margin-bottom": "20px" })}"><div class="shimmer-bg" style="${ssrRenderStyle({ "width": "40px", "height": "40px", "border-radius": "10px", "background": "rgba(255,255,255,0.07)" })}"></div><div class="shimmer-bg" style="${ssrRenderStyle({ "width": "52px", "height": "20px", "border-radius": "20px", "background": "rgba(255,255,255,0.05)" })}"></div></div><div class="shimmer-bg" style="${ssrRenderStyle({ "width": "80px", "height": "30px", "border-radius": "8px", "background": "rgba(255,255,255,0.08)", "margin-bottom": "8px" })}"></div><div class="shimmer-bg" style="${ssrRenderStyle({ "width": "120px", "height": "11px", "border-radius": "6px", "background": "rgba(255,255,255,0.05)" })}"></div></div>`);
        });
        _push(`<!--]-->`);
      } else {
        _push(`<!--[-->`);
        ssrRenderList(unref(kpis), (k) => {
          _push(`<div style="${ssrRenderStyle({ "border-radius": "16px", "background": "linear-gradient(145deg,#0C1A2E,#06101E)", "border": "1px solid rgba(255,255,255,0.07)", "padding": "22px 24px", "position": "relative", "overflow": "hidden", "box-shadow": "0 4px 20px rgba(0,0,0,0.4)", "transition": "transform 0.2s" })}"><div style="${ssrRenderStyle({ position: "absolute", top: "-20px", right: "-20px", width: "80px", height: "80px", borderRadius: "50%", background: k.glow, filter: "blur(24px)", pointerEvents: "none" })}"></div><div style="${ssrRenderStyle({ "display": "flex", "align-items": "flex-start", "justify-content": "space-between", "margin-bottom": "18px" })}"><div style="${ssrRenderStyle({ width: "40px", height: "40px", borderRadius: "10px", background: k.gradient, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 14px ${k.glow}` })}">`);
          ...
          _push(`</div>`);
          if (k.delta !== null) {
            _push(`<div style="${ssrRenderStyle({ display: "flex", alignItems: "center", gap: "3px", fontSize: "11px", fontWeight: 600, padding: "3px 8px", borderRadius: "20px", color: k.delta > 0 ? "#22C55E" : k.delta < 0 ? "#EF4444" : "#94a3b8", background: k.delta > 0 ? "rgba(34,197,94,0.1)" : k.delta < 0 ? "rgba(251,113,133,0.1)" : "rgba(123,146,176,0.1)" })}">`);
            if (k.delta > 0) {
          ...
          _push(`</div><div style="${ssrRenderStyle({ "font-size": "28px", "font-weight": "800", "color": "#E2EAF4", "letter-spacing": "-0.5px", "line-height": "1", "margin-bottom": "6px" })}">${ssrInterpolate(k.value)}</div><div style="${ssrRenderStyle({ "font-size": "12px", "font-weight": "500", "color": "rgba(100,118,142,0.85)" })}">${ssrInterpolate(k.label)}</div><div style="${ssrRenderStyle({ "font-size": "11px", "color": "rgba(100,118,142,0.75)", "margin-top": "3px" })}">${ssrInterpolate(k.sub)}</div><div style="${ssrRenderStyle({ marginTop: "18px", height: "2px", borderRadius: "999px", background: `linear-gradient(90deg,${k.accent}50,transparent)` })}"></div></div>`);
        });
        _push(`<!--]-->`);
      }
      _push(`</div><div style="${ssrRenderStyle({ display: "grid", gridTemplateColumns: unref(isMobile) ? "1fr" : "1fr 300px", gap: "16px", alignItems: "start" })}"><div style="${ssrRenderStyle({ "border-radius": "16px", "background": "linear-gradient(145deg,#0C1A2E,#06101E)", "border": "1px solid rgba(255,255,255,0.07)", "overflow": "hidden", "box-shadow": "0 4px 20px rgba(0,0,0,0.4)" })}"><div style="${ssrRenderStyle({ "display": "flex", "align-items": "center", "justify-content": "space-between", "padding": "16px 20px", "border-bottom": "1px solid rgba(255,255,255,0.06)" })}"><div style="${ssrRenderStyle({ "display": "flex", "align-items": "center", "gap": "8px" })}">`);
      _push(ssrRenderComponent(unref(Receipt), {
        size: 14,
        color: "#7DD3FC",
        "stroke-width": 1.8
      }, null, _parent));
      _push(`<span style="${ssrRenderStyle({ "font-size": "13px", "font-weight": "700", "color": "#E2EAF4" })}">Facturas recientes</span>`);
      if (!unref(loading)) {
        _push(`<span style="${ssrRenderStyle({ "font-size": "11px", "padding": "2px 7px", "border-radius": "20px", "background": "rgba(14,165,233,0.1)", "color": "#7DD3FC", "font-weight": "600" })}">${ssrInterpolate(unref(totalOrders))}</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/orders",
        style: { "display": "flex", "align-items": "center", "gap": "4px", "font-size": "12px", "color": "#7DD3FC", "font-weight": "500" }
      }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(` Ver todas `);
      ...
      if (!unref(loading) && unref(recent).length) {
        ...
        ssrRenderList(["Folio", "Fecha", "Total"], (h) => {
          _push(`<div style="${ssrRenderStyle({ fontSize: "10px", fontWeight: 600, color: "rgba(100,118,142,0.8)", textTransform: "uppercase", letterSpacing: "0.7px", textAlign: h === "Total" ? "right" : "left" })}">${ssrInterpolate(h)}</div>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<!---->`);
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(recent), (f, i) => {
          _push(ssrRenderComponent(_component_NuxtLink, {
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<div style="${ssrRenderStyle([{ "display": "grid", "grid-template-columns": "1fr 110px 90px", "padding": "12px 20px", "transition": "background 0.15s", "cursor": "pointer" }, { borderBottom: i < unref(recent).length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }])}"${_scopeId}><div style="${ssrRenderStyle({ "display": "flex", "align-items": "center", "gap": "8px", "min-width": "0" })}"${_scopeId}><div style="${ssrRenderStyle({ width: "6px", height: "6px", borderRadius: "50%", background: unref(facturaStatusStyle)(f.estatus ?? f.estado).dot, boxShadow: `0 0 6px ${unref(facturaStatusStyle)(f.estatus ?? f.estado).dot}80`, flexShrink: 0 })}"${_scopeId}></div><span style="${ssrRenderStyle({ "font-size": "12px", "font-weight": "600", "color": "#7B92B0", "font-family": "monospace", "overflow": "hidden", "text-overflow": "ellipsis", "white-space": "nowrap" })}"${_scopeId}>${ssrInterpolate(f.folio || "—")}</span></div><div style="${ssrRenderStyle({ "font-size": "11px", "color": "rgba(100,118,142,0.75)", "align-self": "center" })}"${_scopeId}>${ssrInterpolate(unref(fmtDate)(f.fecha_creacion))}</div><div style="${ssrRenderStyle({ "text-align": "right", "align-self": "center" })}"${_scopeId}><div style="${ssrRenderStyle({ "font-size": "12px", "font-weight": "700", "color": "#E2EAF4" })}"${_scopeId}>${ssrInterpolate(unref(parseTotal)(f.total) > 0 ? unref(fmtCompact)(unref(parseTotal)(f.total)) : "—")}</div>`);
                if (f.moneda) {
                  _push2(`<div style="${ssrRenderStyle({ "font-size": "10px", "color": "rgba(100,118,142,0.7)" })}"${_scopeId}>${ssrInterpolate(f.moneda)}</div>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</div></div>`);
                ...
                return [
                  createVNode("div", {
                    ...
                    createVNode("div", { style: { "display": "flex", "align-items": "center", "gap": "8px", "min-width": "0" } }, [
                      createVNode("div", {
                        style: { width: "6px", height: "6px", borderRadius: "50%", background: unref(facturaStatusStyle)(f.estatus ?? f.estado).dot, boxShadow: `0 0 6px ${unref(facturaStatusStyle)(f.estatus ?? f.estado).dot}80`, flexShrink: 0 }
                      }, null, 4),
                      createVNode("span", { style: { "font-size": "12px", "font-weight": "600", "color": "#7B92B0", "font-family": "monospace", "overflow": "hidden", "text-overflow": "ellipsis", "white-space": "nowrap" } }, toDisplayString(f.folio || "—"), 1)
                    ]),
                    createVNode("div", { style: { "font-size": "11px", "color": "rgba(100,118,142,0.75)", "align-self": "center" } }, toDisplayString(unref(fmtDate)(f.fecha_creacion)), 1),
                    createVNode("div", { style: { "text-align": "right", "align-self": "center" } }, [
                      createVNode("div", { style: { "font-size": "12px", "font-weight": "700", "color": "#E2EAF4" } }, toDisplayString(unref(parseTotal)(f.total) > 0 ? unref(fmtCompact)(unref(parseTotal)(f.total)) : "—"), 1),
                      f.moneda ? (openBlock(), createBlock("div", {
                        key: 0,
                        style: { "font-size": "10px", "color": "rgba(100,118,142,0.7)" }
      ...
      if (!unref(loading) && !unref(recent).length) {
        _push(`<div style="${ssrRenderStyle({ "padding": "40px 20px", "text-align": "center", "font-size": "13px", "color": "rgba(100,118,142,0.6)" })}"> Sin facturas para ${ssrInterpolate(unref(YEAR))}</div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div><div style="${ssrRenderStyle({ "display": "flex", "flex-direction": "column", "gap": "14px" })}"><div style="${ssrRenderStyle({ "border-radius": "16px", "background": "linear-gradient(145deg,#0C1A2E,#06101E)", "border": "1px solid rgba(255,255,255,0.07)", "padding": "18px 20px", "box-shadow": "0 4px 20px rgba(0,0,0,0.4)" })}"><div style="${ssrRenderStyle({ "display": "flex", "align-items": "flex-start", "justify-content": "space-between", "margin-bottom": "18px" })}"><div><div style="${ssrRenderStyle({ "display": "flex", "align-items": "center", "gap": "6px", "margin-bottom": "4px" })}">`);
      _push(ssrRenderComponent(unref(BarChart3), {
        size: 13,
        color: "rgba(100,118,142,0.7)",
        "stroke-width": 1.8
      }, null, _parent));
      _push(`<span style="${ssrRenderStyle({ "font-size": "11px", "font-weight": "600", "color": "rgba(100,118,142,0.7)", "text-transform": "uppercase", "letter-spacing": "0.7px" })}">Gasto ${ssrInterpolate(unref(YEAR))}</span></div><div style="${ssrRenderStyle({ "font-size": "20px", "font-weight": "800", "color": "#E2EAF4", "letter-spacing": "-0.3px" })}">${ssrInterpolate(unref(loading) ? "—" : unref(fmtCompact)(unref(totalSpent)))}</div></div>`);
      if (!unref(loading) && unref(spentDelta) !== 0) {
        _push(`<div style="${ssrRenderStyle({ display: "flex", alignItems: "center", gap: "3px", fontSize: "11px", fontWeight: 600, padding: "3px 8px", borderRadius: "20px", color: unref(spentDelta) > 0 ? "#22C55E" : "#EF4444", background: unref(spentDelta) > 0 ? "rgba(34,197,94,0.1)" : "rgba(251,113,133,0.1)" })}">`);
...
//# sourceMappingURL=dashboard-ClPBfnZo.js.map
