<template>
  <div :style="{ fontFamily:`'Inter',system-ui,sans-serif`, display:'flex', flexDirection:'column', gap:'22px' }">
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
      <div>
        <h1 style="font-size:22px;font-weight:800;color:#E2EAF4;margin:0;line-height:1.25;">
          Bienvenido,&nbsp;
          <span style="background:linear-gradient(135deg,#0EA5E9,#7DD3FC);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">
            {{ auth.user?.name.split(' ')[0] }}
          </span>
        </h1>
        <p style="font-size:12px;color:rgba(100,118,142,0.75);margin:5px 0 0;text-transform:capitalize;">{{ dateStr }}</p>
      </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;">
      ...
      <template v-if="loading">
        <div v-for="i in 3" :key="i" style="border-radius:16px;background:linear-gradient(145deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:22px 24px;">
      ...
      <div v-else v-for="k in kpis" :key="k.label"
        style="border-radius:16px;background:linear-gradient(145deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:22px 24px;position:relative;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.4);transition:transform 0.2s;"
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:18px;">
          ...
          <div v-if="k.delta !== null" :style="{ display:'flex', alignItems:'center', gap:'3px', fontSize:'11px', fontWeight:600, padding:'3px 8px', borderRadius:'20px', color:k.delta>0?'#22C55E':k.delta<0?'#EF4444':'#94a3b8', background:k.delta>0?'rgba(34,197,94,0.1)':k.delta<0?'rgba(251,113,133,0.1)':'rgba(123,146,176,0.1)' }">
            <TrendingUp v-if="k.delta>0" :size="10" /><TrendingDown v-else-if="k.delta<0" :size="10" />
            {{ k.delta !== 0 ? `${Math.abs(k.delta)}%` : 'Sin cambio' }}
          </div>
        </div>
        <div style="font-size:28px;font-weight:800;color:#E2EAF4;letter-spacing:-0.5px;line-height:1;margin-bottom:6px;">{{ k.value }}</div>
        <div style="font-size:12px;font-weight:500;color:rgba(100,118,142,0.85);">{{ k.label }}</div>
        <div style="font-size:11px;color:rgba(100,118,142,0.75);margin-top:3px;">{{ k.sub }}</div>
    ...
    <div :style="{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 300px', gap:'16px', alignItems:'start' }">
      ...
      <!-- Facturas recientes -->
      <div style="border-radius:16px;background:linear-gradient(145deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.4);">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid rgba(255,255,255,0.06);">
          <div style="display:flex;align-items:center;gap:8px;">
            <Receipt :size="14" color="#7DD3FC" :stroke-width="1.8" />
            <span style="font-size:13px;font-weight:700;color:#E2EAF4;">Facturas recientes</span>
            <span v-if="!loading" style="font-size:11px;padding:2px 7px;border-radius:20px;background:rgba(14,165,233,0.1);color:#7DD3FC;font-weight:600;">{{ totalOrders }}</span>
          </div>
          <NuxtLink to="/orders" style="display:flex;align-items:center;gap:4px;font-size:12px;color:#7DD3FC;font-weight:500;">
        ...
        <div v-if="!loading && recent.length" style="display:grid;grid-template-columns:1fr 110px 90px;padding:8px 20px;border-bottom:1px solid rgba(255,255,255,0.04);">
          <div v-for="h in ['Folio','Fecha','Total']" :key="h" :style="{ fontSize:'10px', fontWeight:600, color:'rgba(100,118,142,0.8)', textTransform:'uppercase', letterSpacing:'0.7px', textAlign:h==='Total'?'right':'left' }">{{ h }}</div>
        </div>
        ...
        <NuxtLink v-else v-for="(f, i) in recent" :key="`${f.folio}-${i}`" to="/orders" style="text-decoration:none;">
          <div style="display:grid;grid-template-columns:1fr 110px 90px;padding:12px 20px;transition:background 0.15s;cursor:pointer;"
            ...
            <div style="display:flex;align-items:center;gap:8px;min-width:0;">
              ...
              <span style="font-size:12px;font-weight:600;color:#7B92B0;font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ f.folio || '—' }}</span>
            </div>
            <div style="font-size:11px;color:rgba(100,118,142,0.75);align-self:center;">{{ fmtDate(f.fecha_creacion) }}</div>
            <div style="text-align:right;align-self:center;">
              <div style="font-size:12px;font-weight:700;color:#E2EAF4;">{{ parseTotal(f.total)>0 ? fmtCompact(parseTotal(f.total)) : '—' }}</div>
              <div v-if="f.moneda" style="font-size:10px;color:rgba(100,118,142,0.7);">{{ f.moneda }}</div>
            </div>
          </div>
        </NuxtLink>

        <div v-if="!loading && !recent.length" style="padding:40px 20px;text-align:center;font-size:13px;color:rgba(100,118,142,0.6);">
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <!-- Bar chart -->
        <div style="border-radius:16px;background:linear-gradient(145deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:18px 20px;box-shadow:0 4px 20px rgba(0,0,0,0.4);">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:18px;">
            <div>
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;">
                <BarChart3 :size="13" color="rgba(100,118,142,0.7)" :stroke-width="1.8" />
                <span style="font-size:11px;font-weight:600;color:rgba(100,118,142,0.7);text-transform:uppercase;letter-spacing:0.7px;">Gasto {{ YEAR }}</span>
              </div>
              <div style="font-size:20px;font-weight:800;color:#E2EAF4;letter-spacing:-0.3px;">{{ loading ? '—' : fmtCompact(totalSpent) }}</div>
            </div>
            <div v-if="!loading && spentDelta !== 0" :style="{ display:'flex', alignItems:'center', gap:'3px', fontSize:'11px', fontWeight:600, padding:'3px 8px', borderRadius:'20px', color:spentDelta>0?'#22C55E':'#EF4444', background:spentDelta>0?'rgba(34,197,94,0.1)':'rgba(251,113,133,0.1)' }">
          ...
          <div style="display:flex;align-items:flex-end;gap:4px;height:72px;">
            <div v-for="(bar, i) in chartBars" :key="i" :title="`${bar.month}: ${fmtCompact(bar.amount)}`"
              ...
              <div :style="{ fontSize:'8px', fontWeight:bar.current?700:400, color:bar.current?'#7DD3FC':'rgba(100,118,142,0.5)' }">{{ bar.month }}</div>
            </div>
          </div>
        </div>
        ...
        <!-- Summary -->
        <div style="border-radius:16px;background:linear-gradient(145deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:18px 20px;box-shadow:0 4px 20px rgba(0,0,0,0.4);">
          <div style="font-size:11px;font-weight:600;color:rgba(100,118,142,0.7);text-transform:uppercase;letter-spacing:0.7px;margin-bottom:14px;">Resumen</div>
          <div style="display:flex;flex-direction:column;gap:10px;">
            <div v-for="row in summaryRows" :key="row.label" style="display:flex;justify-content:space-between;align-items:center;">
              <span style="font-size:12px;color:rgba(100,118,142,0.75);">{{ row.label }}</span>
              <span style="font-size:12px;font-weight:700;color:#E2EAF4;">{{ row.value }}</span>
            </div>
          </div>
          ...
          <template v-if="!loading && topStatuses.length">
            ...
            <div style="font-size:11px;font-weight:600;color:rgba(100,118,142,0.7);text-transform:uppercase;letter-spacing:0.7px;margin-bottom:10px;">Por estatus</div>
            <div style="display:flex;flex-direction:column;gap:8px;">
              <div v-for="[label, count] in topStatuses" :key="label">
                <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                  ...
                  <span style="font-size:11px;color:rgba(100,118,142,0.7);">{{ count }}</span>
                </div>
