<template>
  <div :style="{ fontFamily:`'Inter',system-ui,sans-serif` }">
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px;">
      <div>
        <h1 style="font-size:22px;font-weight:800;color:#E2EAF4;margin:0;">Gestión de Usuarios</h1>
    ...
    <div style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:14px 16px;margin-bottom:20px;display:flex;flex-direction:column;gap:12px;">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;">
        <button v-for="f in roleFilters" :key="f.key" @click="roleFilter=f.key"
          :style="{ height:'30px', padding:'0 12px', borderRadius:'20px', fontSize:'11px', fontWeight: roleFilter===f.key ? 600:500, cursor:'pointer', border:'none', fontFamily:'inherit', background: roleFilter===f.key ? 'linear-gradient(135deg,#0EA5E9,#0284C7)':'rgba(255,255,255,0.05)', color: roleFilter===f.key ? '#fff':'rgba(100,118,142,0.9)', boxShadow: roleFilter===f.key ? '0 3px 10px rgba(14,165,233,0.25)':'none' }">
          {{ f.label }}
        </button>
      </div>
    </div>
    ...
    <div v-if="loading" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;">
      <div v-for="i in 6" :key="i" class="pulse" style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);height:180px;" />
    </div>
    ...
    <div v-else style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;">
      <div v-for="user in filtered" :key="user.id"
        style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:18px;position:relative;box-shadow:0 3px 14px rgba(0,0,0,0.3);transition:box-shadow 0.25s,transform 0.2s;"
        ...
        <div v-if="isAdmin" style="position:absolute;top:14px;right:14px;">
          <button @click.stop="openMenu===user.id ? openMenu=null : openMenu=user.id"
            style="width:28px;height:28px;border-radius:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);display:flex;align-items:center;justify-content:center;cursor:pointer;color:rgba(100,118,142,0.7);">
            <MoreVertical :size="13" />
          </button>
          <Transition name="dropdown">
            <div v-if="openMenu===user.id" @click.stop style="position:absolute;right:0;top:34px;width:140px;background:#0C1A2E;border:1px solid rgba(255,255,255,0.1);border-radius:12px;box-shadow:0 12px 32px rgba(0,0,0,0.5);overflow:hidden;z-index:20;">
              <button @click="openEdit(user)"
                style="width:100%;display:flex;align-items:center;gap:8px;padding:9px 12px;background:none;border:none;cursor:pointer;font-size:12px;color:#7B92B0;font-family:inherit;"
              ...
              <button @click="startDelete(user.id)"
                style="width:100%;display:flex;align-items:center;gap:8px;padding:9px 12px;background:none;border:none;cursor:pointer;font-size:12px;color:#EF4444;font-family:inherit;"
                @mouseenter="e => (e.currentTarget as HTMLElement).style.background='rgba(239,68,68,0.06)'"
                @mouseleave="e => (e.currentTarget as HTMLElement).style.background='transparent'">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px;">
          ...
          <span v-if="user.discountPct && user.discountPct > 0" style="font-size:10px;font-weight:700;padding:3px 9px;border-radius:20px;background:rgba(34,197,94,0.12);color:#22C55E;border:1px solid rgba(34,197,94,0.25);">
            -{{ user.discountPct }}%
          </span>
        </div>
        ...
        <div v-if="user.lastLogin" style="display:flex;flex-direction:column;gap:7px;padding-top:12px;border-top:1px solid rgba(255,255,255,0.06);">
          <div style="display:flex;justify-content:space-between;">
            <span style="font-size:11px;color:rgba(100,118,142,0.75);">Último acceso</span>
    ...
    <div v-if="!loading && filtered.length===0 && !apiError" style="display:flex;align-items:center;justify-content:center;padding:60px 0;border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);font-size:14px;font-weight:600;color:#94a3b8;margin-top:4px;">
    ...
    <Transition name="modal">
      <div v-if="modal" style="position:fixed;inset:0;z-index:1051;display:flex;align-items:center;justify-content:center;padding:16px;">
        ...
        <div style="position:relative;">
          <div style="position:absolute;inset:-1px;border-radius:20px;background:linear-gradient(135deg,rgba(14,165,233,0.4),rgba(14,165,233,0.25),rgba(14,165,233,0.1));z-index:0;pointer-events:none;" />
          <div style="position:relative;z-index:1;border-radius:20px;background:linear-gradient(160deg,#0C1A2E,#06101E);padding:28px 28px 24px;box-shadow:0 32px 80px rgba(0,0,0,0.7);">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:22px;">
              <div>
                <div style="font-size:16px;font-weight:700;color:#E2EAF4;">{{ modal==='create' ? 'Nuevo usuario' : 'Editar usuario' }}</div>
                <div style="font-size:12px;color:rgba(100,118,142,0.8);margin-top:3px;">{{ modal==='create' ? 'Completa los datos para agregar un usuario' : `Modificando a ${editTarget?.name.split(' ')[0]}` }}</div>
              </div>
              <button @click="!saving && (modal=null)" style="width:30px;height:30px;border-radius:8px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);display:flex;align-items:center;justify-content:center;cursor:pointer;color:rgba(100,118,142,0.8);">
                <X :size="14" />
              </button>
            </div>
            ...
            <form @submit.prevent="handleSave" style="display:flex;flex-direction:column;gap:14px;">
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                ...
                <div style="grid-column:1/-1;">
                  <div style="font-size:11px;font-weight:600;color:rgba(100,118,142,0.9);margin-bottom:6px;letter-spacing:0.3px;">Descuento en catálogo (%)</div>
                  <div style="position:relative;">
                    <input v-model.number="form.discountPct" type="number" min="0" max="100" step="0.5" placeholder="0"
                      style="width:100%;height:40px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:0 40px 0 14px;font-size:13px;color:#E2EAF4;outline:none;font-family:inherit;box-sizing:border-box;transition:border-color 0.2s;"
                    ...
                    <span style="position:absolute;right:13px;top:50%;transform:translateY(-50%);font-size:13px;color:rgba(100,118,142,0.6);pointer-events:none;">%</span>
                  </div>
                  <div style="font-size:11px;color:rgba(100,118,142,0.5);margin-top:5px;">0 = precio normal · 10 = 10% de descuento sobre precios SYSCOM</div>
                </div>
              </div>

              <Transition name="fade">
                <div v-if="formError" style="padding:9px 12px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.25);font-size:12px;color:#EF4444;">{{ formError }}</div>
              </Transition>
  ...
  <Transition name="modal">
    <div v-if="confirmDeleteId" style="position:fixed;inset:0;z-index:1061;display:flex;align-items:center;justify-content:center;padding:16px;pointer-events:none;">
      <div style="width:100%;max-width:360px;pointer-events:auto;">
        <div style="border-radius:20px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(239,68,68,0.3);padding:28px 24px 24px;box-shadow:0 32px 80px rgba(0,0,0,0.7);display:flex;flex-direction:column;align-items:center;gap:10px;text-align:center;">
          <div style="width:48px;height:48px;border-radius:14px;background:rgba(239,68,68,0.12);border:1px solid rgba(239,68,68,0.25);display:flex;align-items:center;justify-content:center;margin-bottom:4px;">
            <Trash2 :size="22" color="#EF4444" />
          </div>
          <div style="font-size:16px;font-weight:700;color:#E2EAF4;">¿Eliminar a {{ deleteTarget?.name.split(' ')[0] }}?</div>
          <div style="font-size:13px;color:rgba(100,118,142,0.85);line-height:1.5;">
            Se eliminará <strong style="color:#7B92B0;">{{ deleteTarget?.name }}</strong> del sistema.<br>Esta acción no se puede deshacer.
          </div>
          ...
          <div style="display:flex;gap:10px;margin-top:10px;width:100%;">
            ...
            <button @click="handleDelete()" :disabled="deleting || isSelf" :style="{ flex:1, height:'40px', borderRadius:'10px', border:'none', background: (deleting||isSelf)?'rgba(239,68,68,0.35)':'linear-gradient(135deg,#f43f5e,#e11d48)', color:'white', fontSize:'13px', fontWeight:600, cursor:(deleting||isSelf)?'not-allowed':'pointer', fontFamily:'inherit', boxShadow:(deleting||isSelf)?'none':'0 4px 16px rgba(239,68,68,0.35)' }">
