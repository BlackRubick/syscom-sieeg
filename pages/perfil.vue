<template>
  <div :style="{ fontFamily:`'Inter',system-ui,sans-serif`, maxWidth:'860px', margin:'0 auto' }">
    ...
    <div style="position:relative;border-radius:20px;overflow:hidden;margin-bottom:20px;box-shadow:0 8px 32px rgba(0,0,0,0.4);">
      ...
      <div style="position:absolute;inset:0;background:linear-gradient(160deg,#0C1A2E,#06101E);z-index:0;opacity:0.7;" />
      ...
      <div style="position:relative;z-index:2;padding:32px 32px 28px;display:flex;align-items:flex-end;gap:24px;flex-wrap:wrap;">
        ...
        <div style="position:relative;flex-shrink:0;">
          ...
          <!-- Indicador de estado -->
          <div :style="{ position:'absolute', bottom:'-3px', right:'-3px', width:'18px', height:'18px', borderRadius:'50%', background: auth.user?.status==='active' ? '#22C55E':'#f43f5e', border:'2px solid #06101E', boxShadow:`0 0 8px ${auth.user?.status==='active'?'#22C55E':'#f43f5e'}99` }" />
        </div>

        <!-- Nombre y datos -->
        <div style="flex:1;min-width:0;">
          <div style="font-size:24px;font-weight:800;color:#E2EAF4;letter-spacing:-0.5px;line-height:1.1;">{{ auth.user?.name }}</div>
          <div style="font-size:13px;color:rgba(123,146,176,0.75);margin-top:5px;display:flex;align-items:center;gap:6px;">
          ...
          <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
            ...
            <span :style="{ fontSize:'11px', fontWeight:600, padding:'4px 12px', borderRadius:'20px', background: auth.user?.status==='active'?'rgba(34,197,94,0.15)':'rgba(239,68,68,0.15)', color: auth.user?.status==='active'?'#22C55E':'#EF4444', border:`1px solid ${auth.user?.status==='active'?'rgba(34,197,94,0.25)':'rgba(239,68,68,0.25)'}`, display:'flex', alignItems:'center', gap:'5px' }">
              <span :style="{ width:'5px', height:'5px', borderRadius:'50%', background: auth.user?.status==='active'?'#22C55E':'#f43f5e', display:'inline-block' }" />
              {{ auth.user?.status === 'active' ? 'Activo' : 'Inactivo' }}
            </span>
            <!-- #5 — Cambiar contraseña -->
            <button @click="pwOpen=true" style="height:26px;padding:0 10px;border-radius:20px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);color:rgba(123,146,176,0.8);font-size:10px;font-weight:600;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:5px;">
        ...
        <div style="display:flex;gap:2px;background:rgba(0,0,0,0.25);border-radius:14px;border:1px solid rgba(255,255,255,0.07);overflow:hidden;flex-shrink:0;">
          <div v-for="(stat, i) in quickStats" :key="stat.label"
            ...
            <div style="font-size:13px;font-weight:700;color:#E2EAF4;white-space:nowrap;">{{ stat.value }}</div>
            <div style="font-size:10px;color:rgba(100,118,142,0.7);margin-top:2px;white-space:nowrap;">{{ stat.label }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 2fr;gap:16px;align-items:start;">
      ...
      <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <!-- Estado fiscal -->
        <div style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:20px;text-align:center;">
          <div :style="{ width:'52px', height:'52px', borderRadius:'16px', margin:'0 auto 12px', display:'flex', alignItems:'center', justifyContent:'center', background: auth.user?.fiscalCompleted ? 'rgba(34,197,94,0.12)' : 'rgba(245,158,11,0.1)', border: `1px solid ${auth.user?.fiscalCompleted ? 'rgba(34,197,94,0.25)' : 'rgba(245,158,11,0.2)'}` }">
            <svg v-if="auth.user?.fiscalCompleted" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#22C55E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
          ...
          <div style="font-size:14px;font-weight:700;color:#E2EAF4;margin-bottom:4px;">
            {{ auth.user?.fiscalCompleted ? 'Datos completos' : 'Datos pendientes' }}
          </div>
          <div style="font-size:12px;color:rgba(100,118,142,0.7);line-height:1.5;margin-bottom:16px;">
          ...
          <button @click="showModal=true"
            :style="{ width:'100%', height:'38px', borderRadius:'10px', border:'none', background: auth.user?.fiscalCompleted ? 'rgba(14,165,233,0.1)' : 'linear-gradient(135deg,#F59E0B,#D97706)', color: auth.user?.fiscalCompleted ? '#7DD3FC' : 'white', fontSize:'12px', fontWeight:600, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', justifyContent:'center', gap:'6px', border: auth.user?.fiscalCompleted ? '1px solid rgba(14,165,233,0.2)' : 'none' }">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" :stroke="auth.user?.fiscalCompleted?'#7DD3FC':'white'" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg>
            {{ auth.user?.fiscalCompleted ? 'Actualizar datos' : 'Completar ahora' }}
          </button>
        </div>

        <!-- RFC chip (solo si tiene datos) -->
        <div v-if="auth.user?.fiscalRfc" style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:18px;">
          <div style="font-size:10px;font-weight:700;color:rgba(100,118,142,0.5);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">RFC</div>
          <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
            <span style="font-size:15px;font-weight:700;color:#7DD3FC;font-family:monospace;letter-spacing:1px;">{{ auth.user.fiscalRfc }}</span>
            <button @click="copyRFC" :title="copied?'¡Copiado!':'Copiar RFC'"
              :style="{ width:'30px', height:'30px', borderRadius:'8px', border:'none', background: copied?'rgba(34,197,94,0.15)':'rgba(255,255,255,0.05)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', flexShrink:0, transition:'all 0.2s' }">
              <svg v-if="!copied" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(123,146,176,0.7)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
          ...
          <div v-if="auth.user.fiscalRazonSocial" style="font-size:11px;color:rgba(100,118,142,0.6);margin-top:8px;line-height:1.4;">{{ auth.user.fiscalRazonSocial }}</div>
        </div>

        <!-- Régimen fiscal chip -->
        <div v-if="auth.user?.fiscalRegimen" style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);padding:18px;">
          <div style="font-size:10px;font-weight:700;color:rgba(100,118,142,0.5);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">Régimen Fiscal</div>
          ...
          <div style="font-size:12px;color:rgba(123,146,176,0.75);line-height:1.4;">{{ regimenNombre(auth.user.fiscalRegimen) }}</div>
        </div>
      ...
      <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <!-- Sin datos fiscal -->
        <div v-if="!auth.user?.fiscalCompleted" style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(245,158,11,0.2);overflow:hidden;">
          <div style="padding:32px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:14px;">
            ...
            <div>
              ...
              <div style="font-size:13px;color:rgba(100,118,142,0.75);line-height:1.6;max-width:320px;margin:0 auto;">
            ...
            <div style="display:flex;flex-direction:column;gap:8px;width:100%;max-width:280px;">
              <div v-for="item in pendingItems" :key="item" style="display:flex;align-items:center;gap:8px;text-align:left;">
                ...
                <span style="font-size:12px;color:rgba(123,146,176,0.7);">{{ item }}</span>
              </div>
            </div>
            <button @click="showModal=true"
        ...
        <template v-else>
          ...
          <div style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);overflow:hidden;">
            <div style="padding:14px 20px;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;align-items:center;gap:8px;">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#7DD3FC" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="16" height="20" x="4" y="2" rx="2"/><path d="M8 10h8"/><path d="M8 14h5"/><path d="M8 6h8"/></svg>
          ...
          <!-- Domicilio fiscal -->
          <div v-if="hasDomicilio" style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);overflow:hidden;">
            <div style="padding:14px 20px;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;align-items:center;gap:8px;">
            ...
            <div style="padding:18px 20px;">
              ...
              <div v-if="direccionFormateada" style="font-size:13px;color:#7B92B0;line-height:1.7;margin-bottom:14px;padding:12px 14px;border-radius:10px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);">
                {{ direccionFormateada }}
              </div>
          ...
          <!-- Contacto adicional -->
          <div v-if="hasContacto" style="border-radius:16px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.07);overflow:hidden;">
            <div style="padding:14px 20px;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;align-items:center;gap:8px;">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#22C55E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 11.5a19.36 19.36 0 0 1-3-8.59A2 2 0 0 1 3.64 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 8.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
  ...
  <Teleport to="body">
    ...
    <Transition name="modal">
      <div v-if="pwOpen" style="position:fixed;inset:0;z-index:1041;display:flex;align-items:center;justify-content:center;padding:16px;">
        <div style="width:100%;max-width:420px;border-radius:20px;background:linear-gradient(160deg,#0C1A2E,#06101E);border:1px solid rgba(255,255,255,0.1);box-shadow:0 32px 80px rgba(0,0,0,0.7);overflow:hidden;">
          <div style="padding:20px 22px;border-bottom:1px solid rgba(255,255,255,0.07);">
            <div style="font-size:16px;font-weight:800;color:#E2EAF4;">Cambiar contraseña</div>
          </div>
          <div style="padding:20px 22px;display:flex;flex-direction:column;gap:14px;">
            <div v-if="pwError" style="padding:9px 12px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);font-size:12px;color:#EF4444;">{{ pwError }}</div>
            <div v-if="pwSuccess" style="padding:9px 12px;border-radius:8px;background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.2);font-size:12px;color:#22C55E;">{{ pwSuccess }}</div>
            <div style="display:flex;flex-direction:column;gap:6px;">
              <label style="font-size:11px;font-weight:600;color:rgba(100,118,142,0.8);">Contraseña actual</label>
              <input v-model="pwCurrent" type="password" placeholder="••••••••"
                style="height:38px;border-radius:9px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);color:#E2EAF4;font-size:13px;padding:0 12px;outline:none;font-family:inherit;" />
            </div>
            <div style="display:flex;flex-direction:column;gap:6px;">
              <label style="font-size:11px;font-weight:600;color:rgba(100,118,142,0.8);">Nueva contraseña</label>
              <input v-model="pwNew" type="password" placeholder="Mínimo 8 caracteres"
                style="height:38px;border-radius:9px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);color:#E2EAF4;font-size:13px;padding:0 12px;outline:none;font-family:inherit;" />
            </div>
            <div style="display:flex;flex-direction:column;gap:6px;">
              <label style="font-size:11px;font-weight:600;color:rgba(100,118,142,0.8);">Confirmar nueva contraseña</label>
