<template>
  <div style="font-family:'Inter',system-ui,sans-serif;min-height:80vh;display:flex;align-items:center;justify-content:center;padding:24px;">
    ...
    <div v-else-if="status === 'success'" style="display:flex;flex-direction:column;align-items:center;text-align:center;gap:4px;">
      <div style="width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,#22C55E,#34D399);display:flex;align-items:center;justify-content:center;margin-bottom:16px;box-shadow:0 0 40px rgba(34,197,94,0.3);">
      ...
      <div v-if="authorization" style="font-size:11px;font-family:monospace;color:rgba(100,118,142,0.45);margin-bottom:22px;">Autorización: {{ authorization }}</div>
    ...
    <div v-else-if="status === 'failed'" style="display:flex;flex-direction:column;align-items:center;text-align:center;gap:4px;">
      ...
      <div style="font-size:22px;font-weight:800;color:#E2EAF4;margin-bottom:8px;">Pago no realizado</div>
      ...
      <div v-if="errorMsg" style="font-size:12px;color:rgba(100,118,142,0.65);max-width:300px;line-height:1.6;margin-bottom:22px;">{{ errorMsg }}</div>
    ...
    <div v-else style="display:flex;flex-direction:column;align-items:center;text-align:center;gap:4px;">
      <div style="font-size:18px;font-weight:700;color:#E2EAF4;margin-bottom:6px;">No se encontró información del pago</div>
      <div style="font-size:13px;color:rgba(100,118,142,0.65);margin-bottom:22px;">Si realizaste un pago, revisa el estado de tu orden.</div>
